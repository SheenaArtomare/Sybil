...
static float		g_Alpha;								// フィールドの不透明度
							...
							int nNumBlockX, int nNumBlockZ, float fBlockSizeX, float fBlockSizeZ)
{
	...
	for (int z = 0; z < (g_nNumBlockZField + 1); z++)
	{
		for (int x = 0; x < (g_nNumBlockXField + 1); x++)
		{
			...
			//g_Vertex[z * (g_nNumBlockXField + 1) + x].Position.y = RandomFloat(2, 2.0f, 0.0f);
...
{
	//int pattern = GetParticlePattern();
	int pattern = 0;

	// エフェクトパターンの合わせてテクスチャに切り替え
	SetFieldTex(pattern);
...
void DrawMeshField(void)
{
	...
	material.Diffuse = { 1.0f, 1.0f, 1.0f, g_Alpha };
...
void SetFieldTex(int pattern)
{
	...
	if (pattern == pattern)
	{
		...
		if (g_TexNo == pattern && g_Alpha < 1.0f)
		{
			g_Alpha += 0.01f;
		}
	}
